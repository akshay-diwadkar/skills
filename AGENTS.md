...
3. **Packaging Boundaries**: Skill folders under `skills/` contain ONLY runtime-required files. Development tests, evals, and fixtures MUST reside under `tests/`.
4. **Lifecycle as Metadata**: Do not create maturity folders (`experimental/`, `stable/`). Store maturity state in catalog YAML files.
