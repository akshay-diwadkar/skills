...
def _git_changes(root: Path, revision: str) -> tuple[set[str], str] | None:
    ...
    if not revision or revision == "unknown":
        return set(), current
    outputs = [
        _git(root, "diff", "--name-status", revision, "HEAD"),
        _git(root, "diff", "--name-status"),
        _git(root, "diff", "--cached", "--name-status"),
    ]
...
def check_freshness(repo_root: Path | str, knowledge_dir: Path | str | None = None) -> dict[str, Any]:
    ...
    if any(not (out / name).is_file() for name in REQUIRED):
        return {
            "status": "missing",
            "reason": "Required knowledge artifacts are missing.",
            "changed_files": [],
    ...
    if validate_schema_json(manifest, "manifest.schema.json") or any(
        ...
        return {
            "status": "invalid",
            "reason": "Manifest/schema or symbol shard invalid.",
            "changed_files": [],
            "requires_full_rebuild": True,
        }
    if manifest.get("schema_version") != SCHEMA_VERSION or manifest.get("extractor_version") != EXTRACTOR_VERSION:
        return {
            "status": "stale",
            "reason": "Schema or extractor version changed.",
            "changed_files": [],
            "requires_full_rebuild": True,
        }
    if manifest.get("config_hash", manifest.get("ignore_hash")) != _config_hash(config):
        return {
            "status": "stale",
            "reason": "Indexing configuration or discovery rules changed.",
            "changed_files": [],
            "requires_full_rebuild": True,
        }
    ...
    for path in sorted(candidates):
        if (output_relative and (path == output_relative or path.startswith(output_relative + "/"))) or matches_glob(
            path, config.get("exclude", [])
        ):
            continue
        full = root / path
        if not full.exists():
    ...
    if current_revision != manifest.get("repository", {}).get("revision") and not changed:
        return {
            ...
            "reason": "Repository revision changed without indexed file changes.",
            "changed_files": [],
            "requires_full_rebuild": False,
        }
    return {
        "status": "fresh" if not changed else "partially-stale",
        "reason": "No relevant repository changes" if not changed else f"{len(changed)} relevant repository changes.",
...
def refresh_knowledge(
    ...
    if state.get("requires_full_rebuild"):
        ...
        return {
            ...
            "status": "fresh",
            "reason": state["reason"],
            "changed_files": explicit or state.get("changed_files", []),
            "details": result,
        }
...
def _refresh_revision_only(root: Path, out: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "mode": "incremental",
        "status": "fresh",
        "reason": "Updated repository revision metadata.",
        "changed_files": [],
    }
...
def _apply_delta(root: Path, out: Path, config: dict[str, Any], changes: list[str]) -> None:
    ...
    for path in changes:
        ...
        unknowns: list[str] = []
        if suffix == ".py":
            extracted, raw_imports, _, unknowns = extract_python_file(full, path, content, subsystem)
        elif suffix in {".js", ".jsx", ".ts", ".tsx"}:
            extracted, raw_imports, _, unknowns = extract_javascript_file(full, path, content, subsystem)
        elif suffix in {".go", ".rs", ".java", ".c", ".cpp"}:
            extracted, raw_imports, _, unknowns = extract_lexical_file(full, path, content, subsystem)
        symbols = [
            {
                "name": item.name,
    ...
    test_links = [
        {"source": file["path"], "target": edge["target"], "kind": "test"}
        for file in files_list
        if file["role"] == "test"
        for edge in imports
    ...
    entry_points = [
        {"path": file["path"], "symbol": (file["symbols"][0] if file["symbols"] else "main"), "kind": "entry-point"}
        for file in files_list
        if Path(file["path"]).name.lower() in {"main.py", "app.py", "index.ts", "index.js", "server.js", "cli.py"}
    ]
    repo.update(
        {
            "files": files_list,
    ...
    relationships = {
        ...
        "calls": [],
        "test_links": sorted(test_links, key=lambda item: (item["source"], item["target"])),
        "configuration_links": [],
        "unresolved_imports": sorted(unresolved, key=lambda item: (item["source"], item["import"])),
        "reverse_imports": {key: sorted(value) for key, value in sorted(reverse.items()) if value},
    }
    ...
    catalog = {
        "schema_version": SCHEMA_VERSION,
        "symbol_count": sum(item["count"] for item in old_shards.values()),
        "shards": [old_shards[key] for key in sorted(old_shards)],
    }
    ...
    manifest.update(
        {
            ...
            "file_hashes": {item["path"]: item["hash"] for item in files_list},
            "artifact_hashes": {name: _digest(data) for name, data in artifacts.items()},
            "changed_files": changes,
            "freshness_state": "fresh",
        }
    )
    manifest["index_hash"] = _digest(manifest["artifact_hashes"])
    manifest["inventory_hash"] = _digest(manifest["file_hashes"])
    write_file_deterministic(out / "manifest.json", serialize_json_deterministic(manifest))
    languages = sorted({item["language"] for item in files_list if item["language"]})
    write_file_deterministic(
        out / "context.md",
        format_context_md(
            revision, subsystems, languages, entry_points, repo.get("commands", []), unknowns=repo.get("unknowns", [])
        ),
    )
    write_file_deterministic(
        out / "architecture.md",
