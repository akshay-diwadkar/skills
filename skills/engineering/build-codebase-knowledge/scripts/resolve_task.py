...
def _signals(task: str) -> dict[str, set[str]]:
    ...
    paths = {item.replace("\\", "/") for item in raw if "/" in item or re.search(r"\.[A-Za-z0-9]{1,5}$", item)}
    symbols = {item for item in raw if re.search(r"[A-Z]|_", item) and "/" not in item}
    terms: set[str] = set()
    for item in raw + quoted:
        terms.update(_split(item))
    return {"paths": paths, "symbols": symbols, "terms": terms}
...
def _score(files: list[dict[str, Any]], relationships: dict[str, Any], signals: dict[str, set[str]], config: dict[str, Any], freshness: str) -> list[dict[str, Any]]:
    ...
    for file in files:
        ...
        if any(path == value or path.endswith("/" + value) for value in signals["paths"]):
            evidence[f"exact_path: {path}"] = weights["exact_path"]
        file_terms = set(_split(path))
        filename_terms = set(_split(Path(path).stem))
        ...
        if exact:
            evidence[f"exact_symbol: {exact[0]}"] = weights["exact_symbol"]
        for label, matched, weight in (("filename_match", signals["terms"] & filename_terms, "filename"), ("symbol_token", signals["terms"] & symbol_terms, "symbol_token"), ("text_match", signals["terms"] & file_terms, "text_match")):
            if matched:
                evidence[f"{label}: {sorted(matched)[0]}"] = weights[weight]
        if file.get("generated"):
            evidence["generated_file_penalty"] = weights["generated_penalty"]
        if "vendor" in path.lower() or "node_modules" in path.lower():
            evidence["vendor_file_penalty"] = weights["vendor_penalty"]
        if not file.get("language") and file["role"] == "source":
            evidence["unsupported_extractor_penalty"] = weights["unsupported_extractor_penalty"]
        if freshness != "fresh":
            evidence["stale_knowledge_penalty"] = weights["stale_knowledge_penalty"]
        score = sum(evidence.values())
        if score > 0:
            candidates.append({"file": file, "score": score, "evidence": evidence, "exact": bool(exact)})
...
def _load_symbols(directory: Path, catalog: dict[str, Any], paths: set[str]) -> dict[str, list[dict[str, Any]]]:
    selected = {_shard_id(path) for path in paths}
    symbols: dict[str, list[dict[str, Any]]] = {path: [] for path in paths}
    for shard in catalog.get("shards", []):
        if shard["id"] not in selected:
            continue
        for symbol in json.loads((directory / shard["path"]).read_text(encoding="utf-8"))["symbols"]:
            if symbol["path"] in symbols:
...
def _target(root: Path, candidate: dict[str, Any], symbols: list[dict[str, Any]], signals: dict[str, set[str]]) -> dict[str, Any]:
    file = candidate["file"]
    match = next((item for item in symbols if item["name"] in signals["symbols"]), None)
    if match is None:
        match = next((item for item in symbols if signals["terms"] & set(_split(item["name"]))), None)
    start = end = None
    if match:
        start, end = match["line_start"], match["line_end"]
    else:
        try:
            ...
            if matches:
                start, end = max(1, matches[0] - 3), min(len(lines), matches[0] + 6)
        except OSError:
            pass
...
def resolve_task(repo_root: Path | str, task: str, knowledge_dir: Path | str | None = None, phase: int | str = 1) -> dict[str, Any]:
    ...
    directory, _, repo, relationship_data = _load_base(root, Path(knowledge_dir).resolve() if knowledge_dir else None)
    config = load_config(root)
    catalog = relationship_data.pop("_catalog")
    ...
    selected_paths = {item["file"]["path"] for item in primary_candidates}
    symbol_map = _load_symbols(directory, catalog, selected_paths)
    ...
    score = primary_candidates[0]["score"] if primary_candidates else 0
    margin = score - (primary_candidates[1]["score"] if len(primary_candidates) > 1 else 0)
    ...
    if phase == "all":
        payload["phases"] = [{"phase": number, **value} for number, value in phases.items()]
    else:
        payload.update({"phase": int(phase), **phases[int(phase)]})
...
def main() -> int:
    ...
    result = resolve_task(args.repo_root, args.task, args.output, args.phase if args.phase == "all" else int(args.phase))
