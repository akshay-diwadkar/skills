...
2. Parse metadata before content:
   - Require `<!-- plan-contract: 5 -->`, strict metadata, the complete typed record graph, targeted repository binding, and a valid v5 `finalize_plan.py` receipt.
   - Reject all other versions and unversioned plans with `contract.unsupported`.
