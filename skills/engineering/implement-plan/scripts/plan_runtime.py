...
def repo_snapshot(root: Path) -> dict[str, Any]:
    ...
    for path in sorted(p for p in root.rglob("*") if p.is_file() and ".git" not in p.parts):
        files.append({"path": path.relative_to(root).as_posix(), "sha256": sha256_bytes(path.read_bytes())})
...
def snapshot_matches(root: Path, baseline: dict[str, Any]) -> bool:
    return canonical_json(repo_snapshot(root)) == canonical_json(baseline)
...
def finalized_text(text: str, binding: dict[str, Any]) -> str:
    ...
    lines.insert(insertion, receipt_line(canonical_text(text), binding))
    return "\n".join(lines) + "\n"
