...
REQUIRED_SECTIONS = ("Outcome", "Obligations", "Evidence", "Implementation", "Verification")
SECTION_ORDER = (
    "Outcome",
    "Obligations",
    "Evidence",
...
REFERENCE_FIELDS = {
    ...
    "CH": {"evidence": {"F"}, "depends_on": {"CH"}, "owner": {"F", "CH"}},
    "P": {"owner": {"CH"}},
    "B": {"evidence": {"F"}},
    "R": {"owner": {"CH"}, "tests": {"T"}},
    "T": {"covers": {"SC", "CH"}},
}
RECORD_RE = re.compile(r"^\s*(?:-\s+)?(?P<id>(?:SC|RQ|F|D|CH|P|B|R|T)-[1-9]\d*): (?P<body>\S.*?)\s*$")
RECORD_LIKE_RE = re.compile(r"^\s*(?:-\s*)?(?P<prefix>SC|RQ|F|D|CH|P|B|R|T)-")
RECORD_TARGET_RE = re.compile(r"^\s*(?:-\s*)?(?P<record>(?:SC|RQ|F|D|CH|P|B|R|T)-[^\s:|]*)")
ID_RE = re.compile(r"\b(?:SC|RQ|F|D|CH|P|B|R|T)-[1-9]\d*\b")
...
def _record_diagnostics(plan: Plan) -> list[Diagnostic]:
    ...
    for record in plan.all_records():
        ...
        for field, allowed in REFERENCE_FIELDS.get(kind, {}).items():
            ...
            if field in REQUIRED_FIELDS[kind] and not refs and record.fields.get(field, "") != "none":
        ...
        if kind == "RQ":
            for field, minimum in (("source", 1), ("anchor", 2), ("obligation", 2)):
                if not _substantive(record.fields.get(field, ""), minimum):
                    diagnostics.append(Diagnostic("record.invalid", f"Obligation {field} must be concrete.", f"Make {record.id}.{field} concrete.", record.id, line=record.line))
            covered_by = _refs(record.fields.get("covered_by", ""))
            if not (covered_by & all_ids and covered_by & plan.ids("SC") and covered_by & (plan.ids("CH") | plan.ids("T"))):
                diagnostics.append(
                    Diagnostic(
                        "obligation.coverage",
                        "Obligations must be covered by at least one success criterion and one change or verification record.",
                        f"Extend {record.id}.covered_by with one SC and one CH or T reference.",
                        record.id,
                        line=record.line,
                    )
                )
        if kind == "CH":
            ...
            if record.fields.get("locality", "") not in {"local", "shared", "test-only"}:
            ...
            if record.fields.get("reversibility", "") not in {"reversible", "conditional", "irreversible"}:
            ...
            if record.fields.get("propagation", "") not in {"", "local", "none"}:
                diagnostics.append(Diagnostic("record.invalid", "Change propagation must be local or none when declared.", f"Correct {record.id}.propagation.", record.id, line=record.line))
            if record.fields.get("reversibility") == "irreversible" and (
                plan.tier != "high-risk" or "irreversible-external-effect" not in plan.risk_domains
            ):
                diagnostics.append(
                    Diagnostic(
                        "record.invalid",
                        "Irreversible changes require high-risk tier and the irreversible-external-effect domain.",
                        f"Set {record.id} reversibility to conditional, or raise the tier and declare the domain.",
                        record.id,
                        line=record.line,
                    )
                )
    ...
    diagnostics.extend(_change_graph_diagnostics(plan))
    return diagnostics


def change_execution_order(plan: Plan) -> list[str]:
    """Return CH ids in deterministic dependency order, earliest dependencies first."""
    changes = {record.id: record for record in plan.records.get("CH", ())}
    edges: dict[str, set[str]] = {
        identifier: set(_refs(record.fields.get("depends_on", ""))) & set(changes) for identifier, record in changes.items()
    }
    ...
    while remaining:
        ...
        if not ready:
            raise ValueError("cycle in change dependency graph")
        identifier = ready[0]
...
def _change_graph_diagnostics(plan: Plan) -> list[Diagnostic]:
    diagnostics: list[Diagnostic] = []
    changes = {record.id: record for record in plan.records.get("CH", ())}
    all_changes = set(changes)
    for identifier, change in changes.items():
        dependencies = set(_refs(change.fields.get("depends_on", "")))
        if identifier in dependencies:
            diagnostics.append(
                Diagnostic(
                    "change.dependency",
                    "A change cannot depend on itself.",
                    f"Remove {identifier} from {identifier}.depends_on.",
                    identifier,
                    line=change.line,
                )
            )
        if dependencies and not dependencies & all_changes:
            diagnostics.append(
                Diagnostic(
                    "change.dependency",
                    "Change dependencies must reference declared changes.",
                    f"Correct {identifier}.depends_on references.",
                    identifier,
                    line=change.line,
                )
            )
        if change.fields.get("locality") == "shared" and plan.tier != "tiny":
            owned = any(
                identifier in _refs(propagation.fields.get("owner", ""))
            ...
            if not owned:
                diagnostics.append(
                    Diagnostic(
                        ...
                        "Shared changes require a matching propagation record.",
                        f"Add a P-n record with owner: {identifier}.",
                        identifier,
                        line=change.line,
                    )
                )
        if plan.tier != "tiny" and change.fields.get("locality") != "test-only":
            declared = change.fields.get("propagation", "")
            owned = any(
                identifier in _refs(propagation.fields.get("owner", ""))
                for propagation in plan.records.get("P", ())
            )
            if not owned and declared not in {"local", "none"}:
                diagnostics.append(
                    Diagnostic(
                        "propagation.accounting",
                        "Non-tiny changes require propagation accounting.",
                        f"Add a P-n record with owner: {identifier} or declare propagation: local|none on {identifier}.",
                        identifier,
                        line=change.line,
                    )
                )
            if declared in {"local", "none"} and not _refs(change.fields.get("evidence", "")):
                diagnostics.append(
                    Diagnostic(
                        "propagation.accounting",
                        "Declared propagation needs evidence backing.",
                        f"Reference same-path evidence from {identifier}.evidence.",
                        identifier,
                        line=change.line,
                    )
                )
    for propagation in plan.records.get("P", ()):
        owners = set(_refs(propagation.fields.get("owner", "")))
        if owners and not owners & all_changes:
            diagnostics.append(
                Diagnostic(
                    "propagation.accounting",
                    "Propagation owners must reference declared changes.",
                    f"Correct {propagation.id}.owner references.",
                    propagation.id,
                    line=propagation.line,
                )
            )
    try:
        change_execution_order(plan)
    except ValueError as exc:
        diagnostics.append(
            Diagnostic(
                "change.dependency",
                f"Change dependency graph is invalid: {exc}.",
                "Break the dependency cycle or repair depends_on references.",
            )
        )
    return diagnostics


def obligation_anchor_diagnostics(plan: Plan, request_text: str) -> list[Diagnostic]:
    """Verify every obligation anchor against the exact request bytes."""
    diagnostics: list[Diagnostic] = []
    for record in plan.records.get("RQ", ()):
        if record.fields.get("anchor", "") not in request_text:
            diagnostics.append(
                Diagnostic(
                    "obligation.anchor",
                    "Obligation anchor is absent from the request.",
                    f"Correct {record.id}.anchor to exact request text.",
                    record.id,
                    line=record.line,
                )
            )
        ...
        "obligations": [
            {
                ...
                "anchor": record.fields["anchor"],
                "anchor_sha256": _sha256(record.fields["anchor"].encode("utf-8")),
                "covered_by": sorted(_refs(record.fields.get("covered_by", ""))),
            }
            for record in plan.records.get("RQ", ())
        ],
        "binding": {
...
def seal_plan(
    ...
    *,
    ...
    if result.valid and result.plan is not None:
        ...
        obligation_diagnostics = obligation_anchor_diagnostics(result.plan, request_text)
        if obligation_diagnostics:
            combined = tuple(result.diagnostics) + tuple(obligation_diagnostics)
            result = ValidationResult(result.plan, combined, result.fact_proofs, result.view)
...
def verify_sealed_plan(text: str, repo_root: Path, *, request_bytes: bytes | None = None) -> tuple[Plan | None, list[Diagnostic], RepositoryView]:
    ...
    legacy_shape = set(proof) == {"version", "facts", "obligations", "binding"}
    enriched_shape = set(proof) == {"version", "facts", "obligations", "binding", "request"}
    request_shape_valid = (
        legacy_shape
        or enriched_shape
    ...
    if validation.valid and validation.plan is not None:
        ...
        if not proof_matches_body:
            diagnostics.append(
                Diagnostic(
                    "proof.stale",
                    "Proof facts, obligations, or bound files do not match the validated plan body and repository.",
        ...
        if proof_shape_valid and request_bytes is not None and validation.plan is not None:
            try:
                request_text = request_bytes.decode("utf-8")
            except UnicodeDecodeError:
                request_text = request_bytes.decode("utf-8", errors="replace")
            diagnostics.extend(obligation_anchor_diagnostics(validation.plan, request_text))
