#!/usr/bin/env python3
"""Check a design-codebase assessment artifact against Contract v2."""
...
def validate(
    ...
    *,
    ...
    if mode == "targeted":
        ...
        elif assessment.handoff.next != "audit-codebase":
            diagnostics.append(Diagnostic("discovery.only.handoff_invalid", f"Discovery-only mode handoff must be audit-codebase, got '{assessment.handoff.next}'", assessment.handoff.line_number))
    ...
    if assessment.handoff:
        ...
        if h.next == "implement-plan":
            plan_ev = [f for f in assessment.facts if "plan" in f.observation.casefold() or "plan" in f.path.casefold()]
            if not plan_ev:
                diagnostics.append(Diagnostic("handoff.implement.unplanned", "Handoff to implement-plan requires an approved, decision-complete plan in evidence", h.line_number))
