...
- `T-n` — autonomous target-discovery candidate records:
  ...
  - Candidate Score Calculation & Material Tie Threshold:
    Candidate score is computed from ranking enums. In autonomous mode, if `top_score - second_score <= 2.0` (material tie), autonomous selection MUST fail closed and force emission of a `discovery-only` assessment handed off to `audit-codebase`.
...
- `H-n` — assessment-only handoff state (`finish assessment`, `plan-change`, `audit-codebase`, `optimize-codebase`, `implement-plan`).
...
- **Autonomous Mode B:** Perform bounded discovery pass. Generate up to 5 candidate `T-n` records. Rank candidates by scoring enums. Select exactly 1 dominant candidate if top score exceeds second score by $> 2.0$; if tied ($\le 2.0$), emit discovery-only assessment and hand off to `audit-codebase`.
...
- Select exactly one handoff state (`H-n`): `finish assessment`, `plan-change`, `audit-codebase`, `optimize-codebase`, or `implement-plan`.
...
- Use `audit-codebase` when broad repository triage, repository-wide debt inventory, or unselected discovery requires prioritization.
- Use `plan-change` after a target design is approved to produce a file/symbol-level specification.
- Use `optimize-codebase` when the primary issue is measured performance or tooling efficiency.
- Use `implement-plan` ONLY when an approved decision-complete plan already exists.
