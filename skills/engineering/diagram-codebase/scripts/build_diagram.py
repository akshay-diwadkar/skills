"""Build a diagram-codebase HTML file from a JSON payload.
...
def inline_runtime_assets(html):
    ...
    html = html.replace(
        ...
        '<style id="diagram-codebase-style" data-inline-asset="css/style.css">\n'
        + style
        + "\n</style>",
    )
    html = html.replace(
        '<script src="roughjs.js"></script>',
        '<script id="diagram-codebase-roughjs" data-inline-asset="roughjs.js">\n'
        + roughjs
        + "\n</script>",
    )
    return html
...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Build diagram-codebase HTML from a JSON payload.")
