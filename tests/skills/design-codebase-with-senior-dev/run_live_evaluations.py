#!/usr/bin/env python3
"""Run opt-in design-skill evaluations through a provider-neutral adapter."""
...
import score_design_evaluation

DEV_DIR = Path(__file__).resolve().parent
REPO_ROOT = DEV_DIR.parents[2]
SKILL_PATH = REPO_ROOT / "skills" / "engineering" / "design-codebase-with-senior-dev"
...
def invoke_adapter(command: list[str], request: dict[str, Any], timeout_seconds: int) -> str:
    ...
    assessment = response.get("assessment_markdown")
    if not isinstance(assessment, str) or not assessment.strip():
        raise RuntimeError("adapter response must contain non-empty assessment_markdown")
    return assessment
...
def run_case(
    ...
    with tempfile.TemporaryDirectory(prefix=f"design-eval-{case_id}-") as temporary:
        ...
        assessment = invoke_adapter(command, request, timeout_seconds)
        after = tree_hash(fixture)
        result = score_design_evaluation.score(assessment, case, fixture)
        if before != after:
    ...
    (case_dir / f"run-{run_number}.md").write_text(assessment, encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=REPO_ROOT / ".scratch" / "design-codebase-evals")
    parser.add_argument("--expectations", type=Path, default=score_design_evaluation.EXPECTATIONS_PATH)
