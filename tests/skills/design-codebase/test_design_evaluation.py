...
DEV_DIR = REPO_ROOT / "tests" / "skills" / "design-codebase"
SCRIPTS = REPO_ROOT / "skills" / "engineering" / "design-codebase" / "scripts"
...
def build_scenario_assessment(name: str, case: dict) -> str:
    ...
    handoff = case.get("expected_handoff", "finish assessment" if level == "L0" else "plan-change")
...
- Recommended design: Refuse autonomous target selection. Hand off repository-wide triage to audit-codebase.
...
- H-1: status: assessment-only | next: audit-codebase
...
def test_scorer_rejects_negative_semantic_mutations() -> None:
    ...
    for name, case in expectations.items():
        ...
        mutated_raw = (
            raw.replace("plan-change", "implement-plan")
            .replace("finish assessment", "implement-plan")
            .replace("audit-codebase", "implement-plan")
        )
