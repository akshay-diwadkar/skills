...
def test_orphan_ids_and_unmapped_changes_are_rejected() -> None:
    ...
    found = semantic_codes(text, REPO_ROOT / "tests/skills/plan-change/fixtures/tiny")
...
def test_grounded_fact_coverage_is_not_reduced_by_unrelated_bad_citation() -> None:
    ...
    coverage = coverage_summary(text, REPO_ROOT / "tests/skills/plan-change/fixtures/tiny")
...
def test_tiny_blueprint_is_optional_and_supported_when_present() -> None:
    assert "semantic.blueprint.missing" not in semantic_codes(tiny_plan(), REPO_ROOT / "tests/skills/plan-change/fixtures/tiny")
