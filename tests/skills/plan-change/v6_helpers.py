...
RUNTIME_SPEC = importlib.util.spec_from_file_location("plan_change_v6_runtime", SCRIPTS / "plan_runtime.py")
if RUNTIME_SPEC is None or RUNTIME_SPEC.loader is None:
    raise RuntimeError("cannot load plan-change v6 runtime")
...
## Implementation
CH-1: path: src/names.py | anchor: normalize_name | status: existing | evidence: F-1 | change: return the empty string for absent values before stripping present names | locality: local | reversibility: reversible
...
## Implementation
CH-1: path: src/names.py | anchor: normalize_name | status: existing | evidence: F-1 | change: enforce tenant authorization before normalizing any supplied name value | locality: shared | reversibility: reversible
...
## Implementation
CH-1: path: src/names_facade.py | anchor: src package facade owner | status: new | owner: F-1 | change: add a facade that delegates name normalization to the existing package implementation | locality: shared | reversibility: reversible
...
## Implementation
CH-1: path: src/generated_names.py | anchor: generated names output | status: new | owner: F-1 | change: generate an adapter that delegates to the handwritten name normalizer | locality: local | reversibility: reversible
...
<!-- plan-contract: 6 -->
<!-- plan-metadata: {"intent":"feature","tier":"high-risk","risk_domains":["migration","durable-state"]} -->
...
## Implementation
CH-1: path: src/names.py | anchor: normalize_name | status: existing | evidence: F-1 | change: expose an idempotent transformation for the durable-state migration | locality: shared | reversibility: reversible
