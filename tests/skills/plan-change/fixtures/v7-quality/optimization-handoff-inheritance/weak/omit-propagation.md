...
## Implementation
CH-1: path: src/scan.py | anchor: scan | status: existing | evidence: F-1 | depends_on: none | change: hoist the length computation out of the loop while preserving per-iteration ordering | locality: local | reversibility: reversible

## Verification
T-1: covers: SC-1, CH-1 | given: ordered and unordered value lists | when: the targeted scan tests execute | then: the output stays ordered and identical to the prior loop | command: python -m pytest tests/test_scan.py -q
