...
## Verification
T-1: covers: SC-1, CH-1 | given: legacy, migrated, and interrupted-state fixtures | when: migration verification executes twice | then: the suite executes and reports a passing result | command: python -m pytest tests/test_names.py -q
