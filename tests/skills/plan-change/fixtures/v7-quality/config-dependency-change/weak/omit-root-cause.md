...
F-3: kind: source | path: src/requester.py | lines: 1-4 | anchor: timeout | claim: the timeout parameter anchors the adaptation
...
CH-2: path: src/requester.py | anchor: requests | status: existing | evidence: F-3 | depends_on: CH-1 | change: adapt the requester call to the compatibility behavior of the new release | locality: local | reversibility: reversible | propagation: local
