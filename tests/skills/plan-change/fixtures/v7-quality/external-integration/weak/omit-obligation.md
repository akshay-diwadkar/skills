...
## Obligations
RQ-1: source: request | anchor: Add an idempotency key to external payment calls | obligation: the checkout page renders the payment selector | covered_by: SC-1, CH-1
...
## Verification
T-1: covers: SC-1, CH-1 | given: success, ambiguous, and timeout provider responses | when: the targeted billing tests execute | then: ambiguous responses retry exactly once and successful charges stay single | command: python -m pytest tests/test_billing.py -q
