...
REDUCED_MANIFEST = {
    ...
    "lanes": [
        {"id": "quality.full", "workflow": "quality.yml", "job": "quality",
         "args": [
             *baseline.INVENTORY_ARGS,
             "-m", "not fixtures and not benchmark and not benchmark_slow",
         ],
...
def test_merge_recorder_medians_across_runs(tmp_path: Path) -> None:
    ...
    assert merged["slowest_nodes"][0]["node_id"] == "tests/x.py::test_b"
    assert merged["slowest_groups"][0]["suite"] == "x.py"
...
def test_runtime_evidence_contract_accepts_two_run_report() -> None:
    valid = {
        "runtime": {
            ...
            "lane_executions": {"quality.alt": {}},
        },
        "lanes": [{"id": "quality.full"}, {"id": "quality.alt"}],
    }
...
def test_reduced_baseline_is_deterministic(tmp_path: Path) -> None:
    ...
    assert full["node_count"] > 700
    assert first["inventory_source"]["node_count"] >= full["node_count"]
    assert all("map-codebase" not in row["node_id"] for row in first["inventory"])
    assert all("map-codebase" not in row["file"] for row in first["inventory"])
    assert all(row["owning_surface"] for row in first["inventory"])
    assert {row["proof_role"] for row in first["inventory"]} >= {
        "primary-proof",
        "compatibility-evidence",
    }
    assert first["deferred_domains"] == baseline.DEFERRED_DOMAINS
    assert all("map-codebase" not in path for path in first["static"].get("subprocess", []))
    ...
    assert all("map-codebase" not in owner for owner in first["ownership"]["owners"])
